...
// First boot check for wpa_supplicant.conf from Pi imager
if (file_exists('/etc/wpa_supplicant/wpa_supplicant.conf')) {
	// Check for moOde generated file
	...
	if (empty($moodeHeader)) {
		// Extract values from Pi Imager generated wpa_supplicant.conf
		...
		if (!empty($ssid)) {
			// Update cfg_network wlan0 and apd0 rows
			...
			workerLog('worker: WiFi and AP configured from Pi Imager file');
		}
	}
}
