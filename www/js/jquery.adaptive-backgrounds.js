      ...
      c.colors = function (n, t) {
          ...
          e(n, function (e) {
              for (var i = n.width * n.height || e.length, m = {}, s = "", d = [], f = {
                  ...
                  if (d[0] = e[l], d[1] = e[l + 1], d[2] = e[l + 2], s = d.join(","), m[s] = s in m
                      ? m[s] + 1
                      : 1, -1 === c.indexOf(a(s))) {
                      var g = m[s];
                      g > f.dominant.count
                          ? (f.dominant.name = s, f.dominant.count = g)
                          : f
                              .palette
                              .some(function (n) {
                                  return g > n.count
                                      ? (n.name = s, n.count = g, !0)
                                      : void 0
                              })
                  }
                  l += 4 * r
		...
		adaptMback = newcolor;
          ...
          if (opts.normalizeTextColor) {
			...
			if ($('#playback-panel').hasClass('active')) {
				...
				$('#menu-bottom').css({backgroundColor: adaptMback});
          ...
          $parent.addClass(getLumaClass(data.color))
			...
			$('input[type="range"]').removeClass('ab-dark');
			$('input[type="range"]').removeClass('ab-light');
			$('input[type="range"]').addClass(getLumaClass(data.color))
            .attr('data-ab-yaq', getYIQ(data.color));
