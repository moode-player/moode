...
jQuery(document).ready(function($) { 'use strict';
	...
	$("#menu-bottom").delegate("#open-library-panel a", 'click', function(){
		...
		$('#menu-top .dropdown-menu').css({backgroundColor: themeMback});
		// if(!firstClickMusic){
		// 	firstClickMusic = true;
		// 	$('.browse-panel-btn').click();
		// }
	});
	...
	$('.browse-panel-btn').click(function(){
		...
		$('.thumb-panel-btn').removeClass('active');
	...
	$('.thumb-panel-btn').click(function(){		
		$('.library-panel-btn').removeClass('active');
		$('.browse-panel-btn').removeClass('active');
		$('.thumb-panel-btn').addClass('active');
		$('#open-library-panel').html('<a href="#thumb-panel" class="open-library-panel" data-toggle="tab">Music</a>');
		loadThumbs(0);
	});
	...
	// library typedown search
	// newui add click handler for magnifying glass button in library
    $('#lib-search-thumb-btn').click(function(e){		
		if ($('#lib-album-thumb-filter').css('display') == 'inline-block') {
			$('#lib-album-thumb-filter, #lib-album-thumb-filter-results').css('display', 'none');
			$('#lib-album-thumb-filter').val('');
			$('#lib-album-thumb-filter-results').html('');
			clickedLibItem(e, undefined, LIB.filters.artists, renderAlbumsThumb);
		}
		else {
			$('#lib-album-thumb-filter').css('display', 'inline-block');
			$('#lib-album-thumb-filter').focus();
			if ($(window).width() > 479) {
				$('#lib-album-thumb-filter-results').css('display', 'inline-block');
			}
			else {
				$('#lib-album-thumb-filter-results').css('display', 'none');
			}
		}	
	});
	$('#lib-album-thumb-filter').keyup(function(){
		$.scrollTo(0 , 500);
		var filter = $(this).val(), count = 0;	    
		$('.database-thumb li').each(function() {
			if ($(this).text().search(new RegExp(filter, 'i')) < 0) {
				$(this).hide();
			}
			else {
		...
		var s = (count == 1) ? '' : 's';
		if (filter != '') {
			$('#lib-album-thumb-filter-results').html((+count) + '&nbsp;album' + s);
		}
		else {
			$('#lib-album-thumb-filter-results').html('');
		}
	...
	//页面拉到底时自动加载更多
    $(".database-thumb").scroll(function(e) {
			alert(1);
	  });
	...
	$('#ra-search-btn').click(function(){
		if ($('#ra-filter').css('display') == 'inline-block') {
			...
			$('.database-thumb li').each(function(){$(this).show();});
		}
		else {
	...
	$('#screen-saver, #playback-panel, #library-panel, #browse-panel,#thumb-panel #radio-panel, #menu-bottom').click(function() {
