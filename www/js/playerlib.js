...
function renderUI() {
    ...
    $.getJSON('command/cfg-table.php?cmd=get_cfg_system', function(data) {
        ...
        var debugText = SESSION.json['debuglog'] == '1' ? 'Debug logging on' : '';
        debugText += SESSION.json['xss_detect'] == 'on' ? ', XSS detection on' : '';
        if (debugText != '') {
...
function getKeyOrValue (type, item) {
    let mapTable = new Map([
        ...
        ['Shutdown','poweroff'],['Restart','reboot'],['Update library','update_library'],['Refresh screen','refresh_screen'],['Rediscover','rediscover_players'],
        // Root folder icons
