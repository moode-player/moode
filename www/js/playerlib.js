...
var storage=window.localStorage; //lazybat
...
function mpdDbCmd(cmd, path, tagname, uplevel) {
	var cmds = ['add', 'play', 'clrplay', 'addall', 'playall', 'clrplayall', 'update','dspthumb'];
	...
	else if (cmd == 'dspthumb') { // r42e radio tab
		$.post('command/moode.php?cmd=' + 'dspthumb', {'path': path}, function(data) {renderThumb(data, path, uplevel);}, 'json');
	}
...
// render libraryThumb by lazybat
function renderLibraryThumb(data,page) {
    fullLib = data;
	//debugLog(data);
    ...
    // by lazybat
    renderAlbumsThumb(data,page*50);
}
...
function filterLib() {
            ...
            for (var artist in fullLib[genre]) {
                ...
                if (LIB.filters.artists.length == 0 || LIB.filters.artists.indexOf(artist) >= 0) {
                    for (var album in fullLib[genre][artist]) {
                        var objAlbum = {'album': album, 'artist': artist, 'genre': genre,'onefile':fullLib[genre][artist][album][0].file}; // r42z add genre
    ...
    if (needReload) {
			...
			for (var i = 1; i < allAlbumsTmp.length; i++) {
				...
				if (allAlbumsTmp[i].album == allAlbumsTmp[i - 1].album && excludeAlbums.indexOf(allAlbumsTmp[i].album.toLowerCase()) == -1) { // r42z
					...
					if (compAlbumStored == false) {
		                objCompilationAlbum = {'album': allAlbumsTmp[i].album, 'artist': msg, 'compilation': '1','onefile':allAlbumsTmp[i].onefile};
				...
				// current != prev -> lets check 
					...
					// prev is a regular album, store it 
		                ...
		                var objRegularAlbum = {'album': allAlbumsTmp[i - 1].album, 'artist': allAlbumsTmp[i - 1].artist, 'compilation': '0','onefile':allAlbumsTmp[i-1].onefile};
						allAlbums.push(objRegularAlbum);
					}
					// last album
					if (i == allAlbumsTmp.length - 1) {
						// store last album
						var objRegularAlbum = {'album': allAlbumsTmp[i].album, 'artist': allAlbumsTmp[i].artist, 'compilation': '0','onefile':allAlbumsTmp[i].onefile};
						allAlbums.push(objRegularAlbum);
					}
					// reset flag
		...
		// only one album in list
			...
			var objRegularAlbum = {'album': allAlbumsTmp[0].album, 'artist': allAlbumsTmp[0].artist, 'compilation': '0','onefile':allAlbumsTmp[0].onefile}; // store the one and only album
			allAlbums.push(objRegularAlbum);
		}
...
// render albumsThumb by lazybat
var renderAlbumsThumb = function(data,pageindex) {
	// clear search filter and results
	$('#lib-album-thumb-filter').val('');
	$('#lib-album-thumb-filter-results').html('');
	
	var dbList = $('ul.database-thumb');
	if(dbList.html().length <=0 )
		dbList.html('');	
	var tmp = '';
	var pagecount = pageindex+50 > allAlbums.length ? allAlbums.length:pageindex+50;
    ...
    for (var i = pageindex; i < pagecount ; i++) {
		if($('#dbthumb-'+ i).length<=0){
			...
			$('#dbthumb-'+ i).click(function(e){
				
				storage["thumbpath"] = $(this).data('path');
				dindex =  $(this).data('index');
				if (allAlbums[dindex].compilation != '1') {
					// generate song list for regular album		
					clickedLibItem(e, keyAlbum(allAlbums[dindex]), LIB.filters.albums, renderSongs);
				}
				else {
					...
					for (j = 0; j < allSongs.length; j++) {
						if (allSongs[j].album == allAlbums[dindex].album) {
							allCompilationSongs.push(allSongs[j]);
							allFiles.push({'file': allSongs[j].file});
							LIB.totalTime += parseSongTime(allSongs[j].time);
						}
					}
	...
	$("img.lazy").lazyload({         
		effect : "fadeIn",
		container: $(".database-thumb")
	});

	if(pageindex+50 < allAlbums.length){
		setTimeout(function() {	
			renderAlbumsThumb(data,pageindex+50);
		}, 1000);
	}  
...
// click coverart context menu item
$('#context-menu-lib-thumb a').click(function(e) {
	$('#lib-song-' + (UI.dbEntry[0] + 1).toString()).removeClass('active');
	$('img.lib-coverart').css('box-shadow', '0px 0px 3px 3px rgba(0,0,0,0.1)'); // r42x
...
function setColors() {
    ...
    if ($('#radio-panel').hasClass('active') || $('#browse-panel').hasClass('active') || $('#library-panel').hasClass('active')|| $('#thumb-panel').hasClass('active')) {
...
$('#database-thumb').on('scroll', function() {
	if ((SESSION.json['alphablend'] == '1.00') || (themeOp < '.74')) {
		if ($('#database-thumb').scrollTop() > 1 && !showMenuTopR) {
			$('#menu-top').css('height', $('#menu-top').css('line-height'));
			showMenuTopR = true;
		}
		else if ($('#database-thumb').scrollTop() == '0' ) {
			$('#menu-top').css('height', '0');
			showMenuTopR = false;
		}
	}
