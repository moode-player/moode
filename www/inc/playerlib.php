...
function updMpdConf($i2sdevice) {
	...
	$mpdcfg = sqlQuery("SELECT param, value FROM cfg_mpd WHERE value!=''", sqlConnect());
	...
	// ALSA mixer name
	phpSession('write', 'amixname', getMixerName($i2sdevice));
	// Hardware volume
	...
	phpSession('write', 'mpdmixer', $mixer_type);
	phpSession('write', 'mpdmixer_local', $mixer_type);
	...
	$data .= "mixer_type \"" . $mixer_type . "\"\n";
	$data .= $mixer_type == 'hardware' ? "mixer_control \"" . $_SESSION['amixname'] . "\"\n" . "mixer_device \"hw:" . $cardnum . "\"\n" . "mixer_index \"0\"\n" : '';
	$data .= "dop \"" . $dop . "\"\n";
...
// Get ALSA (Hardware) volume
function getALSAVolume($amixname) {
...
function getDeviceNames () {
	...
	if ($_SESSION['i2sdevice'] == 'None' && $_SESSION['i2soverlay'] == 'None') {
		for ($i = 0; $i < 4; $i++) {
			...
			elseif ($alsa_id != 'Loopback' && $alsa_id != 'Dummy') {
				$aplay_device_name = trim(sysCmd("aplay -l | awk -F'[' '/card " . $i . "/{print $2}' | cut -d']' -f1")[0]);
				$result = sqlRead('cfg_audiodev', sqlConnect(), $alsa_id);
...
function sourceCfg($queueargs) {
	...
	switch ($action) {
		...
		case 'delete':
			...
			$return = (sqlDelete('cfg_source', $dbh, $queueargs['mount']['id'])) ? true : false;
			break;
	}
...
function sourceMount($action, $id = '') {
	switch ($action) {
		...
		case 'mountall':
			...
			// sqlRead returns: result set || true = results empty || false = query failed
...
function cfgNetIfaces() {
	...
	// eth0 static
	if ($cfg_network[0]['method'] == 'static') {
		$data .= "interface eth0\n";
		$data .= 'static ip_address=' . $cfg_network[0]['ipaddr'] . '/' . $cfg_network[0]['netmask'] . "\n";
		$data .= 'static routers=' . $cfg_network[0]['gateway'] . "\n";
		$data .= 'static domain_name_servers=' . $cfg_network[0]['pridns'] . ' ' . $cfg_network[0]['secdns'] . "\n";
	}
	// wlan0 static
	if ($cfg_network[1]['method'] == 'static') {
		$data .= "interface wlan0\n";
		$data .= 'static ip_address=' . $cfg_network[1]['ipaddr'] . '/' . $cfg_network[1]['netmask'] . "\n";
		$data .= 'static routers=' . $cfg_network[1]['gateway'] . "\n";
		$data .= 'static domain_name_servers=' . $cfg_network[1]['pridns'] . ' ' . $cfg_network[1]['secdns'] . "\n";
	}
	// wlan0 AP mode
	...
	$data .= "# the player Network configuration page. \n";
	$data .= "#########################################\n\n";
	$data .= 'country=' . $cfg_network[1]['wlan_country'] . "\n";
	...
	if (!empty($cfg_network[1]['wlanssid']) && $cfg_network[1]['wlanssid'] != 'None (activates AP mode)') {
		...
		// Secure
		if ($cfg_network[1]['wlansec'] == 'wpa') {
			$data .= 'psk=' . $cfg_network[1]['wlan_psk'] . "\n";
		}
		// No security
		else {
			$data .= "key_mgmt=NONE\n";
	...
	sysCmd('iw reg set "' . $cfg_network[1]['wlan_country'] . '" >/dev/null 2>&1');
}

// Configure hostapd conf
function cfgHostApd() {
	// Read network config [2] = apd0
	$result = sqlQuery('SELECT * FROM cfg_network', sqlConnect());
...
function loadRadio() {
	...
	$result = sqlRead('cfg_radio', sqlConnect(), 'all');
	foreach ($result as $row) {
